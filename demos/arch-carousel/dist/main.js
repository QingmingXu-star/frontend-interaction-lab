...
const contents = ['content/content_1.JPG', 'content/content_2.PNG', 'content/content_3.JPG'];
let archWidth = 0;
let archHeight = 0;
let gap = 0;
let unitWidth = 0;
...
function getNearestSnapX(currentX) {
    const viewportCenter = window.innerWidth / 2;
...
function triggerZoomThrough(unit) {
    isZooming = true;
    speed = 0;
    targetX = null;
    const contentEl = unit.querySelector('.content-container');
    ...
    const makePillarMask = (leftPx, widthPx) => {
    ...
    const makeSidePanel = (leftPx, widthPx) => {
        const el = document.createElement('div');
        Object.assign(el.style, {
            position: 'absolute',
            top: '0',
