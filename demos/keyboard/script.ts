...
function startAnimation(): void {
    ...
    document.querySelectorAll<HTMLElement>('.key').forEach(key => {
        if (!KEEP_KEYS.includes(key.dataset['k'] ?? '')) {
            setTimeout(() => {
    ...
    setTimeout(() => {
        ...
        const survivors: HTMLElement[] = [
            ...row2.querySelectorAll<HTMLElement>('.key:not(.vanish)'),
            ...row3.querySelectorAll<HTMLElement>('.key:not(.vanish)'),
            ...document.querySelectorAll<HTMLElement>('#row-bottom .key:not(.vanish)'),
        ];
        const firstRects = new Map<HTMLElement, DOMRect>(
            survivors.map(k => [k, k.getBoundingClientRect()])
        );

        // Last — DOM 变更 + 添加收缩态
        [...row3.children].forEach(child => {
            const key = child as HTMLElement;
            key.dataset['k'] === 'caps'
...
function startTyping(): void {
    ...
    const initKey  = document.querySelector<HTMLElement>('[data-k="delete"]')!;
    ...
    function next(): void {
        ...
        const keyEl = document.querySelector<HTMLElement>(`[data-k="${k}"]`);
        if (!keyEl) { idx++; next(); return; }
    ...
    function pressKey(keyEl: HTMLElement, ch: string | null, onRelease: () => void): void {
        ...
        setTimeout(() => {
            if (ch !== null) addChar(output, caret, ch);
    ...
    function sendAfterPause(): void {
        const sendEl = document.querySelector<HTMLElement>('[data-k="send"]')!;
...
/* ─────────────────────────────────────────
   工具函数
───────────────────────────────────────── */
function addChar(output: HTMLElement, caret: HTMLElement, ch: string): void {
